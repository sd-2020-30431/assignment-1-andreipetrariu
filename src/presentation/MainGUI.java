...
public class MainGUI extends Stage{
	...
	public void initGroceries(List<ShopItem> groceries) {
		...
		Button newBtn = new Button("Add Groceries");
		
		Date today = new Date();
		Calendar c = Calendar.getInstance();

		gBottomBox.getChildren().addAll(newBtn,new Label("\t\t\t\t\t"),gBtn,new Label("        \t"));
		gBottomBox.setStyle("-fx-padding: 20px;-fx-border-insets:20px");
		gBottomBox.setAlignment(Pos.BOTTOM_RIGHT);
		gBtn.setStyle("-fx-background-color: lightgreen; -fx-padding:10");
		gBtn.setScaleX(1.5);
		gBtn.setScaleY(1.5);
		gBtn.addEventHandler(MouseEvent.MOUSE_CLICKED, e -> {
		...
		newBtn.setScaleX(1.5);
		newBtn.setScaleY(1.5);
		newBtn.setStyle("-fx-padding: 10");
		newBtn.addEventHandler(MouseEvent.MOUSE_CLICKED, e -> {
			new GroceriesForm(client);
		});

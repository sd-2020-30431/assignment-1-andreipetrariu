...
public class WLDataAccess {
	...
	public boolean register(String username, String password) {
		session = factory.getCurrentSession();
		session.beginTransaction();
		Query query = session.createSQLQuery("select id_users from users where username = :username and password = :password")
				    .setParameter("username", username)
				    .setParameter("password", password);
		if(!query.list().isEmpty()) {
			session.close();
	...
	public void createGroceries(String name, int caloriesInt, int daysInt) {
		session = factory.getCurrentSession();
		session.beginTransaction();
		ShopItem item = new ShopItem(name,caloriesInt,daysInt);
		session.save(item);
		session.getTransaction().commit();
	}
